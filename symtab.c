...
Type* duplicateType(Type* type) { // copy cai type do  -> mot so lenh gan,
...
int compareType(Type* type1, Type* type2) { //so sanh hai kieu voi nhau
...
Object* findObject(ObjectNode *objList, char *name) { //tim kiem name trong list *objList
  while (objList != NULL) {
